namespace CustomPlantClass.Main
{
    public class PlantMgr : MonoBehaviour
    {
        ...
        public static void WaitAndExecute<T>(Action<T> a, T parameter, float delay)
        {
            if (GameAPP.Instance != null) GameAPP.Instance.StartCoroutine(Wait_Internal(a, parameter, delay));
            else throw new InvalidOperationException("Can't do this when the game has not started!");
        }
        private static IEnumerator Wait_Internal<T>(Action<T> a, T parameter, float delay)
        {
