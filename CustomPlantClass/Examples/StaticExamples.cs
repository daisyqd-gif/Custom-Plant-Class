    ...
    public class StaticExamples : MonoBehaviour
    {
        ...
        private static IEnumerator StarShoot_UltimateGatling_Internal(BaseCustomPlant plant, BulletType theBulletType)
        {
            for (int i = 0; i < 5; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    PlantMgr.SetBullet(plant._plant, theBulletType, BulletMoveWay.Right_free, new Vector2(Random.Range(-0.1f, 0.1f), Random.Range(-0.1f, 0.1f)), Random.Range(-15f, 15f)).normalSpeed = Random.Range(12f, 14f);
                }
